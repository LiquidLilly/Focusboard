...
export function BucketColumn({ bucket, project }) {
  ...
  return (
    <div className="flex flex-col w-64 shrink-0">
      {/* Bucket header */}
      <div className="flex items-center justify-between px-2 py-1.5 border border-[#1e1e3a] bg-[#1a1a2e] mb-2">
        {editingName ? (
          <input
            autoFocus
            value={nameValue}
            ...
            className="flex-1 font-mono text-sm bg-transparent outline-none border-b border-[#00fff7] text-[#e0e0e0]"
          />
        ) : (
          <span
            className="font-mono font-semibold text-sm text-[#e0e0e0] cursor-text flex-1 uppercase tracking-wider"
        ...
        <span className="text-xs font-mono text-[#444466] ml-2">{bucket.items.length}</span>
        <div className="flex gap-1 ml-1 opacity-40 hover:opacity-100">
          <button onClick={() => setEditingName(true)} className="text-[#444466] hover:text-[#00fff7]">
            <Edit3 size={11} />
          </button>
          <button onClick={handleDeleteBucket} className="text-[#444466] hover:text-[#ff2020]">
            <Trash2 size={11} />
          </button>
        </div>
      </div>
      ...
      >
        ...
        {/* Add item form */}
        {addingItem ? (
          <div className="flex flex-col gap-1.5 p-1.5 border border-[#1e1e3a] bg-[#1a1a2e]">
            <input
              autoFocus
              value={newItemTitle}
            ...
            <div className="flex gap-1">
              ...
              <button
                ...
                className="text-xs font-mono text-[#444466] hover:text-[#e0e0e0] px-1"
              >
                <X size={10} />
              </button>
            </div>
          ...
          <button
            ...
            className="flex items-center gap-1 text-xs font-mono text-[#444466] hover:text-[#00fff7] px-1 py-1.5 transition-colors"
          >
